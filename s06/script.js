...
//using push methos inside the function
...
nums.sort();
console.log(nums);
//output sorts based of first two digits of the number.
nums.sort(
...
//reverse method
nums.reverse();
console.log(nums);//it reversed the order based on the previous output
count.reverse();
console.log(count);
...
console.log(count);//it displays the elements that was removed from the splice() method.
...
//deletes specified number of elements from index 1 the return the remaining elements in the new array
console.log(newSlice);
...
let elemento = ["b", "a", "d", 8, 5, "e"];
console.log(elemento);
console.log(typeof elemento[2]);//string

let newString = elemento.toString();
...
//NOTE: mapping does not modify the original array
...
//it returns true if a string contains a specified 
...
//Create a function which checks the element in an array, that if given word exist in an array, display the word, if not, display <word> not found.


//Activity #1
// let pets = ["dogs","cats","birds","fishes"];

// function checkArray(arr, word) {
...
// checkArray(pets, "cats"); // "cats"
// checkArray(pets, "rabbits"); // "<rabbits> not found"

//Another
// let pet = ["dogs", "cats", "birds", "fishes"];
// let search = "cats";
// let index2 = pet.indexOf(search);
  
//   if (index !== -1) {
//     console.log(search + " found at index " + index2);
//   } else {
...
//Strech Goal
//Get the average of elements in the array and display the average to your browser's console using forLoop.

//let numeric = [5,11,30,118,634];
// Using reduce() method
// let sum = numeric.reduce((totals, currents) => totals + currents);
// let average = sum / numeric.length;
...
//   sums += numeric[i];
// }
// let averages = sums / numeric.length;
...
//SOLUTION 1
let num = numeric.reduce (
    function (a,b) {
        return a +b
    }
);
console.log(num); //798
console.log(num/numeric.length); //159.6
